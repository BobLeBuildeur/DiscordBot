...
- **SvelteKit** (`orchestration-web`) — `posthog-js` init in root layout or `app.html`, `capture()` on user actions, `$pageview` / `$pageleave` if not automatic.
...
## 4. Implementation plan (instrumentation)

### 4.1 orchestration-web

1. Add `posthog-js` dependency and `PUBLIC_POSTHOG_KEY` (+ optional `PUBLIC_POSTHOG_HOST`).
2. Initialize PostHog once (e.g. `+layout.svelte` or small `lib/analytics/posthog.ts`) **only in browser**, after consent policy if applicable.
3. Emit events from:
...
4. Set **super properties** or `register` once: app version, environment.
5. Use **`session_id` from the API** as a PostHog event property and optionally `group` analytics if you introduce organization groups later.

### 4.2 orchestration-server
...
### 4.4 PostHog Product Analytics (how to use the data)
...
### 4.5 PostHog LLM analytics (how to use the data)
...
- `backend/integrations/openai_client.py` — observability TODO for metadata extraction failures; fold into LLM analytics events as part of §4.2.
