...
class OrchestratorEngine:
    ...
    def start_session(self, problem_statement: str) -> OrchestrationResult:
        ...
        return self._handle_turn(session=session, latest_user_message=problem_statement)
    ...
    def advance_session(self, session_id: str, user_message: str) -> OrchestrationResult:
        ...
        self.store.save_session(session)
        return self._handle_turn(session=session, latest_user_message=user_message)

    def get_session(self, session_id: str) -> SessionState:
        return self.store.load_session(session_id)

    def _handle_turn(self, session: SessionState, latest_user_message: str) -> OrchestrationResult:
    ...
    def _generate_response(
        ...
        if state_check.next_action == NextAction.CREATE_PLAN:
            ...
            return self.llm_client.generate_plan(prompt), "plan"

        prompt = self.prompt_manager.build_plan_refinement_prompt(session, latest_user_message)
        return self.llm_client.refine_plan(prompt), "plan"
