...
| **Books/Rules Storage** | Stored in PocketBase (see 02-knowledge-base) | Extended PocketBase with embedding, vector search |
...
Book/Rule — stored in PocketBase (separate from main app DB)
  └── See 02-knowledge-base for schema, embedding, hybrid search
...
### Book/Rule (PocketBase)

Books (rules) are stored in **PocketBase**, not the main app database. See 02-knowledge-base for full schema and rule retrieval system.

| Column | Type | Description |
|--------|------|-------------|
| id | string | PocketBase record ID |
| name | string | Rule name |
| type | string | Extensible (knowledge, sop, playbook, etc.) |
| content | text | Markdown content |
| embedding | json/blob | Vector embedding for hybrid search. Auto-generated by hook. |
| metadata | json | tags, data_sources, etc. |
| created | datetime | PocketBase audit |
| updated | datetime | PocketBase audit |

PocketBase is extended with: embedding hooks, SQLite vector extension, FTS5 keyword index. MCP exposes `search_rules` for agents.
...
### Phase 3: Data Sources (Books in PocketBase)

10. Books/rules are stored in PocketBase (see 02-knowledge-base). No Books table in main app DB.
...
| **02-knowledge-base** | Books/rules in PocketBase; MCP search_rules |
...
- [ ] DataSources storage (Books in PocketBase per 02-knowledge-base)
